                ...
                for($i = 0; $i < count($people); $i++) {
                  echo '<tr class="item-body">';
                    echo '<td class="item-name">';
                      if ($items['linkToProfile']) echo '<a class="u-url" href="' . esc_url($individualPageUrl . rawurlencode($people[$i]['uid'][0])) . '">';
                        ...
                        echo '<td class="item-info table-renderer">';
                          ...
                          echo '<ul class="inline-list">';
                            if ($value == "Campus Email" || $value == "Other Email") {
                              echo '<li><a href="' . esc_url('mailto:' . $people[$i][$key][0]) . '">' . esc_html($people[$i][$key][0]) . '</a></li>';
                            } else {
              ...
              <div class="section-item h-card wrap">
                  ...
                  <h3 class="item-name">
                    <?php
                      if ($items['linkToProfile']) {
                        echo '<a class="u-url" href="' . esc_url($individualPageUrl . rawurlencode($people[$i]['uid'][0])) . '">';
                      }
                                ...
                                if ($disItem[$key] == "Campus Email" ) {
                                  if (array_key_exists("mail", $people[$i])) {
                                    for($j=0; $j<$people[$i]["mail"]['count']; $j++) {
                                        echo "<li><span><a class=\"u-email kwa\" href=\"" . esc_url('mailto:' . $people[$i]['mail'][$j]) . "\">" . esc_html($people[$i]['mail'][$j]) . "</a></span></li>";
                                    }
                                  }
                                  ...
                                  if (array_key_exists("ucscpersonpubalternatemail", $people[$i])) {
                                    for($j=0; $j<$people[$i]["ucscpersonpubalternatemail"]["count"]; $j++) {
                                        echo "<li><span><a class=\"u-email\" href=\"" . esc_url('mailto:' . $people[$i]['ucscpersonpubalternatemail'][$j]) . "\">" . esc_html($people[$i]['ucscpersonpubalternatemail'][$j]) . "</a></span></li>";
                                    }
                                  }
                                } else if ($disItem[$key] == "Office Location") {
                                  echo "<li>" . esc_html($people[$i]['ucscprimarylocationpubofficialname'][0]) . ", " . esc_html($people[$i]['ucscpersonpubofficelocationdetail'][0]) . "</li>";
                ...
                <?php
                  if ($displayPhoto) {
                    ...
                    if ($items['linkToProfile']) echo '<a class="u-url square-img" href="' . esc_url($individualPageUrl . rawurlencode($people[$i]['uid'][0])) . '">';
