...
if ( $items['dirLayout'] == 'table' ) {
	...
	if ( $items['dirLayout'] == 'tiled' ) {
		?>
		<div class="section-container ucsc-block-directory tiled-page"> 
		<?php
	} else {
		?>
		<div class="section-container ucsc-block-directory list-page"> 
		<?php
	}
	?>
		<div class="section-body">
	...
	for ( $i = 0; $i < count( $people ); $i++ ) {
		...
		foreach ( $items['informationToDisplay'] as $disItem ) {
			foreach ( $disItem as $key => $value ) {
				if ( array_key_exists( $key, $people[ $i ] ) && $people[ $i ][ $key ][0] ) {
					...
					} elseif ( $disItem[ $key ] == 'Other Email' ) {
		...
		if ( $displayPhoto ) {
			...
			if ( $items['linkToProfile'] ) {
				echo '<a class="u-url square-img" href="' . $individualPageUrl . $people[ $i ]['uid'][0] . '">';
			}
			echo "<img src='" . $imgSrc . "' class='item-image square-img imgLiquid imgLiquid_bgSize imgLiquid_ready' style='object-fit: cover;'  alt='Profile picture of " . $people[ $i ]['cn'][0] . "' />";
			if ( $items['linkToProfile'] ) {
				echo '</a>';
			}
		}
