...
class CampusDirectoryAPI {
	...
	function __construct( $content = array() ) {
		...
		$this->allStaffTypes = array(
			'Regular Staff',
			'Researcher',
			'Postdoctoral Scholar',
		);
		$this->ldap_password = get_site_option( 'ldap_api_key' );
		if ( ! $this->ldap_password ) {
			$this->ldap_password = get_option( 'ldap_api_key' );
		}
		$this->ldap_cn = get_site_option( 'ldap_cn' );
		if ( ! $this->ldap_cn ) {
			$this->ldap_cn = get_option( 'ldap_cn' );
		}
		if ( ! $this->ldap_cn ) {
			$this->ldap_cn = 'pantheon-webapps';
		}
	}
	...
	public function getInformationToDisplay() {
		...
		foreach ( $informationTypes as $label => $isSet ) {
			if ( $isSet ) {
				array_push( $returnDictionary, array( $dictFieldInformation[ $label ] => $label ) );
			}
		}
	...
	public function doLDAPQuery( $q, $arrCruzids ) {
		...
		if ( $rli ) {
			...
			if ( $dev_env ) {
				@$ldapbind = ldap_bind( $rli );
			} else {
				@$ldapbind = ldap_bind( $rli, 'cn=' . $this->ldap_cn . ',ou=apps,dc=ucsc,dc=edu', $this->ldap_password );
			}
			if ( $ldapbind ) {
	...
	public function getDirDropdowns( $deptOrDiv ) {
		...
		if ( ! $retDepts ) {
			$people = $this->doLDAPQuery( '(|(ucscpersonpubaffiliation=Graduate)(ucscpersonpubaffiliation=Faculty)(ucscpersonpubaffiliation=Staff))', array() );
	...
	public function addVacantPosition( $position ) {
		...
		return array(
			'cn'    => array( $arrPosition[0] ),
			'title' => array( $arrPosition[1] ),
		);
	}
	...
	public function processFacultyFilterString( &$filterString ) {
		if ( ! empty( $this->nodeContent['objFacultyTypes'] ) ) {
			...
			if ( $facultyTypes['All'] ) {
				...
				++$this->count;
			} elseif ( array_sum( $facultyTypes ) > 0 ) {
