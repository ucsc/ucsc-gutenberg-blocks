...
class Ucsc_Services_Blocks_Campus_Directory_api {
  public $nodeContent;
  ...
  public function Ucsc_Service_Blocks_Set_Directory_Data()
  {
	$variables = [];
	...
	$variables['informationToDisplay'] = $this->Ucsc_Service_Blocks_Get_Information_To_Display();

	$campusDirData = $this->Ucsc_Service_Blocks_Get_Campus_Dir_Data($strCruzids);
  ...
  public function Ucsc_Service_Blocks_Get_Information_To_Display()
  {
  ...
  public function Ucsc_Service_Blocks_Get_Campus_Dir_Data($strCruzids, $profileView = false)
  {
	if ($profileView || !$this->nodeContent['automatedFeeds']) {
	  ...
	  $q = $this->Ucsc_Service_Blocks_Build_Filter_String();
	}
	// check to see if $q is in cache
	$md5_q = md5($q);
	$people = get_transient($md5_q);
	if (!$people) {
	  $people = $this->Ucsc_Service_Blocks_Do_LDAP_Query($q, $arrCruzids);
	  set_transient($md5_q, $people, 600);
	}
	return [$people, $q];
  }

  public function Ucsc_Service_Blocks_Do_LDAP_Query($q, $arrCruzids) {
	...
	if ($rli) {
	  ...
	  if ($ldapbind) {
		...
		$people = $this->Ucsc_Service_Blocks_Process_Search_Results($rli, $sr);
		$people = $this->Ucsc_Service_Blocks_Add_Vacant_Positions($people, $this->nodeContent['automatedFeeds'], $arrCruzids);
		ldap_close($rli);
		return $people;
	  } else {
		echo ldap_error($rli );
  ...
  public function Ucsc_Service_Blocks_Get_Dir_Dropdowns($deptOrDiv) {
	$retDepts = get_transient('ucsc_campus_directory_departments_' . $deptOrDiv);
	if (!$retDepts) {
	  $people = $this->Ucsc_Service_Blocks_Do_LDAP_Query("(|(ucscpersonpubaffiliation=Graduate)(ucscpersonpubaffiliation=Faculty)(ucscpersonpubaffiliation=Staff))", []);
  ...
  public function Ucsc_Service_Blocks_Add_Vacant_Positions($people, $automatedFeed, $arrCruzids)
  {
	$retArr = [];
	if ($automatedFeed) {
	  $retArr = $people;
  ...
  public function Ucsc_Service_Blocks_Build_Filter_String()
  {
	$filterString = "";
	$vacancies = array();
	$this->count = 0;
	...
	$this->Ucsc_Service_Blocks_Process_Faculty_Filter_String($filterString);
	$this->Ucsc_Service_Blocks_Process_Staff_Filter_String($filterString);
	$this->Ucsc_Service_Blocks_Process_Dept_Div_Filter_String($filterString);
	$this->Ucsc_Service_Blocks_Process_Add_Exclude_Filter_String($filterString);

	return $filterString;
  }

  public function Ucsc_Service_Blocks_Process_Faculty_Filter_String(&$filterString)
  {
  ...
  public function Ucsc_Service_Blocks_Process_Staff_Filter_String(&$filterString)
  {
  ...
  public function Ucsc_Service_Blocks_Process_Dept_Div_Filter_String(&$filterString)
  {
  ...
  public function Ucsc_Service_Blocks_Process_Add_Exclude_Filter_String(&$filterString)
  {
  ...
  public function Ucsc_Service_Blocks_Process_Search_Results($rli, $sr)
  {
