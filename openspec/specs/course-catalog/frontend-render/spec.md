...
numeric rank alongside its visible text label, so both can sort correctly
despite being displayed as text. The rank values are: unrecognized/default
level = 0 (sorts first), Lower Division = 1, Upper Division = 2, Graduate = 3
(sorts last). An unrecognized level therefore sorts *before* Lower Division,
not after Graduate.
...
Clicking any cell within a course's header row SHALL toggle visibility of
that course's own description row only, without affecting any other row's
expanded state. The click handler is bound per-cell, not to the row as a
whole, but the effect is that every cell in the row is clickable; the
expand/collapse-all controls are separate elements outside the table and are
