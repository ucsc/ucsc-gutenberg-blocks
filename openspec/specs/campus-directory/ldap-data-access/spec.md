...
> **Known deviation:** as implemented, the "unscoped" 50-record ceiling is only
> applied on the automated-feed code path when neither department nor division
> is selected. `CampusDirectoryAPI::$deptOrDivSet` defaults to `true` and is
> only ever reset to `false` inside `processDeptDivFilterString()`, which the
> manual-CruzID-list code path never calls. A manual-list search — which has
> no organizational scope by definition — therefore currently receives the
> 1000-record ceiling, not 50. Treat the 50-record ceiling as scoped to
> automated feeds only until this is fixed in code; only the 1000-record
> ceiling is exercised by any test today.
...
- **WHEN** an automated feed without a department or division scope is queried
