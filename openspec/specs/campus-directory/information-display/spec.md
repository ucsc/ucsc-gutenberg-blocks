...
> **Known deviation:** "Mailing Address" is defined in
> `CampusDirectoryAPI::getInformationToDisplay()` as the pseudo-field
> `combineaddressfields`, but no render branch for that key exists in
> `CampusDirectoryTemplate.php` for the list, tiled, or table layouts.
> Selecting it in a listing is currently a silent no-op — nothing renders and
> no error is raised. (The profile page renders a mailing address, but through
> unrelated `street`/`roomnumber` fields, not this pseudo-field.) No test
> covers or catches this gap.
