...
surface: the list, tiled, and table layouts, and the profile page.

> **Known deviation (tracked risk):** the `ucsc_profiles` shortcode surface
> (`CampusDirectoryShortcode.php`) does NOT currently satisfy this
> requirement for all fields — `cn`/name and `title` are not fully escaped.
> This is a documented, pre-existing gap: `CampusDirectoryShortcodeTest.php`
> carries four intentionally-failing assertions that record it rather than
> hide it. Treat the shortcode as excluded from this requirement's guarantee
> until that gap is closed; the block's list/tiled/table/profile surfaces are
> not affected and are covered by passing escaping tests.
