...
> **Known deviation:** `biography`, `areas_of_expertise`,
> `research_interests`, `teaching_interests`, `awards`, and `publications` are
> accepted and stored by `CampusDirectoryShortcode`'s attribute parsing, but
> their corresponding rendering code in `render_profiles_grid()` and
> `render_profiles_list()` is commented out. Setting any of these six
> attributes to `true` currently has no visible effect — the field is silently
> not rendered. Only `photo`, `name`, `title`, `phone`, `email`, `websites`,
> `officelocation`, `officehours`, `expertise`, and `profilelinks` are
> functioning today.
