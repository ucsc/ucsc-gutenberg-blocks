...
Instructor deduplication is keyed by instructor name alone for the primary
section's meeting list, but by name **and** CruzID for the secondary
(associated) sections' meeting lists — two instructors sharing a name but
different CruzIDs are treated as one instructor in the primary section and as
two in secondary sections. This asymmetry is intentional-looking but
undocumented in code; treat it as current behavior, not a guarantee.

A meeting time of `"TBA"` (case-insensitive) SHALL NOT be parsed as a clock
time. When the days text is present, the rendered value is the days text
followed by `"TBA"` (e.g. `"MWF TBA"`), not the bare literal string `"TBA"`;
the bare literal only appears when no days text is present either.
...
- **WHEN** a meeting's start or end time is `"TBA"` and days text is present
- **THEN** the rendered time is the days text followed by the literal string
  "TBA" (e.g. "MWF TBA"), not a parsed clock time

> **Testing note:** as of this baseline, no PHP test exercises
> `templates/CourseDetailTemplate.php` at all — this requirement, the
> not-found/error messaging, seat-capacity clamping, and the breadcrumb
> referer logic below are all unverified by automated tests.
