...
function sortTableByColumn(table, column, asc = true) {
	...
	const sortedLinkedRows = rowsLinkedToDesc.sort((a, b) => {
		var special =
			a.row.querySelector(`td:nth-child(${column + 1}) span`) !== null;
		//if needed, the span can have a type to enable special sorting beyond integer
		if (special) {
			const aColText = a.row
				.querySelector(`td:nth-child(${column + 1}) span`)
				.textContent.trim();
			const bColText = b.row
				.querySelector(`td:nth-child(${column + 1}) span`)
				.textContent.trim();
...
document
	.querySelectorAll('.table-sortable td')
	.forEach((tableSortable, index) => {
...
if (collapseAlls)
	collapseAlls.forEach((collapseAll) => {
