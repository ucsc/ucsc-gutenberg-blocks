...
const PeopleAndInformation = ({
	...
	if (typeof automatedFeeds === 'undefined') {
		...
		[localAutomatedFeeds, setLocalAutomatedFeeds] =
			useState(automatedFeeds);
	}
	...
	if (typeof displayDeptartmentAffiliates === 'undefined') {
		[boolDisplayDepartmentAffiliates, setBoolDisplayDepartmentAffiliates] =
			useState(false);
		setAttributes({
	...
	return (
		<div>
			<div className="vertical_radio">
				<RadioControl
					...
					onChange={(newLocalAutomatedFeeds) => {
						setLocalAutomatedFeeds(
							newLocalAutomatedFeeds === 'true'
						);
						setAttributes({
			...
			{!localAutomatedFeeds && (
				<div>
					<h5>
						List Individuals (Enter CruzIDs separated by commas)
					</h5>
					<p>
						<i>
							List individuals in the order you'd like them to
							appear on the page.
						</i>
					</p>
					<TextareaControl
			...
			{localAutomatedFeeds && (
				<div>
					...
					{localManualAdd && (
						<div>
							<h5>
								Add Individuals to the Feed. (Enter CruzIDs
								separated by commas)
							</h5>
							<TextareaControl
							...
							/>
							<h5>
								Exclude Individuals from the Feed. (Enter
								CruzIDs separated by commas)
							</h5>
							<TextareaControl
								value={excludeCruzidsList}
								onChange={(newExcludeCruzidsList) => {
									setExcludeCruzidsList(
										newExcludeCruzidsList
									);
									setAttributes({
					...
					{deptOrDiv === 'dept' && (
						<div>
							<hr />
							<h5>
								Display Department Affiliates? (Rare for
								administrative units.)
							</h5>
							<p>
								<i>
									For example, a faculty member is officially
									associated with the History Department but
									is also affiliated with College Nine because
									they teach a College Nine Core Course.
								</i>
							</p>
